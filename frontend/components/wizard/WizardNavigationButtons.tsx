...
export const WizardNavigationButtons: React.FC = () => {
  ...
  const renderStatus = () => {
    switch (syncStatus) {
      case 'saving':
        return <span className="flex items-center text-xs text-neutral-500 animate-pulse"><Loader2 size={12} className="animate-spin mr-1" /> Saving...</span>;
      case 'saved':
        return <span className="flex items-center text-xs text-green-500"><CheckCircle2 size={12} className="mr-1" /> Draft saved</span>;
      case 'failed':
        return <span className="flex items-center text-xs text-red-500"><AlertCircle size={12} className="mr-1" /> Save failed — retry</span>;
      default:
        return null;
    }
  };
