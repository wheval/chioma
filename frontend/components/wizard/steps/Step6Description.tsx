...
import { FileText, Sparkles, Loader2, Info, MapPin, Bus, LayoutGrid } from 'lucide-react';
...
export const Step6Description: React.FC<StepProps> = ({ data, onChange, errors }) => {
  ...
  const getAiSuggestion = async () => {
    ...
    setIsAiLoading(true);
    try {
      const res = await axios.get(`/api/property-listings/wizard/${draftId}/ai/description-suggestion`);
  ...
  const useSuggestion = (field: 'propertyDescription' | 'neighborhoodDescription') => {
    if (aiSuggestion) {
    ...
    <div className="space-y-10 animate-slide-in">
      ...
      <div className="grid grid-cols-1 gap-10">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-neutral-600 dark:text-neutral-400 flex items-center">
              <Info className="mr-2 text-brand-blue" size={16} /> Property Description (Required)
            </span>
            <span className={`text-[10px] font-bold p-1 rounded-md px-2 ${ (data.propertyDescription?.length || 0) < 50 ? 'bg-red-50 text-red-500' : 'bg-green-50 text-green-500'}`}>
          ...
          <div className="relative group">
            ...
            />
            {aiSuggestion?.propertyDescription && (
              <div className="absolute top-2 right-2 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  onClick={() => useSuggestion('propertyDescription')}
          ...
          {errors.propertyDescription && <span className="text-xs text-red-500 font-medium">{errors.propertyDescription}</span>}
        </div>
        ...
        <div className="space-y-4 pt-6 border-t border-neutral-100 dark:border-neutral-800">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-neutral-600 dark:text-neutral-400 flex items-center">
              <MapPin className="mr-2 text-brand-green" size={16} /> The Neighborhood (Optional)
            </span>
          ...
          <div className="relative group">
            ...
            />
            {aiSuggestion?.neighborhoodDescription && (
              <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                <button 
                  onClick={() => useSuggestion('neighborhoodDescription')}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6">
          <div className="space-y-4">
            <span className="text-sm font-semibold text-neutral-600 dark:text-neutral-400 flex items-center">
              <Bus className="mr-2 text-brand-orange" size={16} /> Transportation Info
            </span>
            <textarea
          ...
          <div className="space-y-4">
            <span className="text-sm font-semibold text-neutral-600 dark:text-neutral-400 flex items-center">
              <LayoutGrid className="mr-2 text-brand-blue" size={16} /> Nearby Amenities
            </span>
            <textarea
