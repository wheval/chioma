        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {RULES.map((rule) => {
            ...
            return (
              ...
              >
                <div className={`mr-4 p-3 rounded-xl transition-all
                  ${isEnabled ? 'bg-brand-blue text-white' : 'bg-neutral-100 dark:bg-neutral-700 text-neutral-400'}`}>
                  {rule.icon}
                </div>
                
                <div className="flex-1">
                  <h3 className={`text-sm font-bold transition-all ${isEnabled ? 'text-brand-blue' : 'text-neutral-800 dark:text-neutral-200'}`}>
                    {rule.label}
                  </h3>
