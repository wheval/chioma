...
import { PropertyListingDraft, PropertyData } from './entities/property-listing-draft.entity';
import { PropertiesService } from './properties.service';
import { ListingStatus, PropertyType } from './entities/property.entity';
...
export class PropertyWizardService {
  ...
  async updateStep(id: string, landlordId: string, step: number, data: Partial<PropertyData>) {
  ...
  async publish(id: string, landlordId: string) {
    ...
    const missingSteps = requiredSteps.filter(s => !draft.completedSteps.includes(s));
    ...
    const property = await this.propertiesService.create(createPropertyDto, landlordId);
  ...
  private validateStep(step: number, data: PropertyData): Record<string, string> {
    ...
    switch (step) {
      case 1:
        ...
        if (data.bedrooms === undefined) errors.bedrooms = 'Bedrooms is required';
        if (data.bathrooms === undefined) errors.bathrooms = 'Bathrooms is required';
      ...
      case 2:
        ...
        if (data.securityDeposit === undefined || data.securityDeposit < 0) errors.securityDeposit = 'Security deposit is required';
      ...
      case 3:
        ...
        break;
      case 5:
        if (!data.photos || data.photos.length < 3) errors.photos = 'At least 3 photos are required';
        break;
      case 6:
        if (!data.propertyDescription || data.propertyDescription.length < 50 || data.propertyDescription.length > 2000) {
          errors.propertyDescription = 'Description must be between 50 and 2000 characters';
        }
        break;
      case 7:
        if (!data.availableFrom) errors.availableFrom = 'Available from date is required';
        if (data.availableFrom && new Date(data.availableFrom) < new Date(new Date().setHours(0,0,0,0))) {
  ...
  private mapPropertyType(type?: string): PropertyType {
    switch (type?.toLowerCase()) {
      ...
      case 'room': return PropertyType.OTHER; // Mapping as example
      case 'studio': return PropertyType.APARTMENT;
      case 'duplex': return PropertyType.HOUSE;
